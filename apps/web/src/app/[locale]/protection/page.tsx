...
export default async function ProtectionPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return (
    ...
    >
      ...
      <h2>{t('Before a sitter appears in search', 'Avant qu’un gardien apparaisse dans la recherche')}</h2>
      ...
      <ul>
        <li>{t('Profiles show a first name and a last initial only.',
               'Les profils n’affichent qu’un prénom et l’initiale du nom.')}</li>
        <li>{t('Maps and search results show an approximate point in a neighbourhood, offset from the real address.',
               'Les cartes et les résultats affichent un point approximatif dans un quartier, décalé de l’adresse réelle.')}</li>
        <li>{t('The exact address is encrypted and released only after a booking is confirmed.',
               'L’adresse exacte est chiffrée et n’est communiquée qu’après la confirmation d’une réservation.')}</li>
        <li>{t('Dates of birth and Social Insurance Numbers are encrypted, access-restricted, and never shown on a profile.',
               'Les dates de naissance et les numéros d’assurance sociale sont chiffrés, à accès restreint, et jamais affichés sur un profil.')}</li>
      </ul>

      <h2>{t('Reviews you can rely on', 'Des avis fiables')}</h2>
      <p>
        {t(
      ...
      <h2>{t('If something goes wrong', 'En cas de problème')}</h2>
      <p>
        {t(
      ...
      <h2>{t('What this is not', 'Ce que ceci n’est pas')}</h2>
      <p>
        {t(
