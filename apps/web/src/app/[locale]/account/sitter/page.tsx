...
import { listSitterBookings, isSitter, isAdmin } from '@/lib/data';
...
export default async function SitterRequestsPage({
  ...
  const [sitter, admin] = await Promise.all([isSitter(session.user.id), isAdmin(session.user.id)]);
  ...
  return (
    <AccountShell locale={locale} title={m.account.requests} active="sitter" isSitter isAdmin={admin}>
      {bookings.length === 0 ? (
