...
import { listSitterBookings, isSitter } from '@/lib/data';
...
export default async function SitterRequestsPage({
  ...
  const sitter = await isSitter(session.user.id);
  ...
  return (
    <AccountShell locale={locale} title={m.account.requests} active="sitter" isSitter>
      {bookings.length === 0 ? (
